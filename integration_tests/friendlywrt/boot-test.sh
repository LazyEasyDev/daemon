...
run_integration_test() {
	...
	phase_file="$state_dir/friendlywrt-phase"
	result_file="$state_dir/friendlywrt-result"
	full_log="$state_dir/friendlywrt-test.log"
	phase_log="$state_dir/friendlywrt-phase.log"
	artifact_archive="$state_dir/friendlywrt-artifacts.tar"
	...
	printf '[friendlywrt-boot] running %s\n' "$phase"
	...
	if [ -d "$install_dir/state-$SERVICE_NAME/artifacts" ]; then
		tar -cf "$artifact_archive" -C "$install_dir/state-$SERVICE_NAME/artifacts" . 2>/dev/null || true
	fi
